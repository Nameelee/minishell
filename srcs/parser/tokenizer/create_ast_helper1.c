...
void attach_suffix_redir(t_ast_state *s)
{
    ...
    // 파이프의 오른쪽에 있는 명령어에 대한 리다이렉션인지 확인
    if ((*(s->root))->token == PIPE)
        target_ptr = &((*(s->root))->right);
    else
        target_ptr = s->root;
    
    // 리다이렉션 노드가 기존 하위 트리의 루트가 됨
...
int handle_pipe(t_ast_state *s)
{
    ...
    s->new_node->left = *(s->root);
    if (*(s->root)) (*(s->root))->parent = s->new_node;
    *(s->root) = s->new_node;
    *(s->csc_head) = NULL;
    return (1);
}
 ...
 int handle_redir(t_ast_state *s, t_token **token_list)
{
    ...
    t_token **attach_target; // 리다이렉션을 붙일 위치(포인터의 주소)
